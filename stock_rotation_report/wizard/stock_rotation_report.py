...
class StockRotationReport(models.TransientModel):
    ...
    def print_report(self):
        ...
        if self.date_from >= self.date_to:
            raise UserError(_("Fecha Inicial es igual o mayor a fecha final."))
        datas = {'form': self.read()[0],
                'get_stock_moves': self.get_stock_moves_details()[0]
            }
    ...
    def stock_rotation_export_excel(self):
        ...
        """
        import base64
        filename = 'Reporte Rotacion de Stock.xls'
        start = self.date_from.strftime('%d %b, %Y')
        end = self.date_to.strftime('%d %b, %Y')
        form_name = 'Rotacion de Stock/Reporte de Movimientos de producto entre ' + str(start) + ' a ' + str(end)
        if self.date_from >= self.date_to:
            raise UserError(_("Fecha Inicial es Mayor a Fecha Final."))
        ...
        #HEADER
        worksheet.write_merge(0, 1, 0, 13, '   Rotacion de Stock/Movimientos de Productos', style = styleheader)
        worksheet.write_merge(2, 2, 0, 1, 'Fecha Inicial : '+ str(self.date_from))
        worksheet.write_merge(3, 3, 0, 1, 'Fecha Final : '+ str(self.date_to))
        #SUB-HEADER
        row = 4
        worksheet.write(row, 0, 'ID Producto', stylecolumnheader)
        worksheet.write(row, 1, 'Nombre Producto', stylecolumnheader)
        worksheet.write(row, 2, 'Referencia Interna', stylecolumnheader)
        worksheet.write(row, 3, 'Precio Costo', stylecolumnheader)
        worksheet.write(row, 4, 'Precio Venta', stylecolumnheader)
        worksheet.write(row, 5, 'Inicial', stylecolumnheader)
        worksheet.write(row, 6, 'Entrante', stylecolumnheader)
        worksheet.write(row, 7, 'Saliente', stylecolumnheader)
        worksheet.write(row, 8, 'Interno', stylecolumnheader)
        worksheet.write(row, 9, 'Cantidad Compra', stylecolumnheader)
        worksheet.write(row, 10, 'Ultima fecha de Compra', stylecolumnheader)
        worksheet.write(row, 11, 'Cantidad Venta', stylecolumnheader)
        worksheet.write(row, 12, 'Ultima Fecha de Venta', stylecolumnheader)
        worksheet.write(row, 13, 'Final', stylecolumnheader)
        row = 5
        data = self.get_stock_moves_details()[0]
        ...
        for warehouse in data:
            ...
            for category in data[warehouse]:
                ...
                for line in data[warehouse][category]:
                    ...
                    worksheet.write(row, 10, str(line.get('last_purchase_date')))
                    worksheet.write(row, 11, line.get('sale_count'))
                    worksheet.write(row, 12, str(line.get('last_sale_date')))
...
class stock_rotation_export_excel(models.TransientModel):
    ...
    _description = "Reporte rotacion de Stock"

    excel_file = fields.Binary('Descargar Rotacion de Stock')
